...
当 Deployment 触发 rollout 动作时，系统会给 Deployment 创建一个新的修订版本（一个新的 ReplicaSet）。意思是仅当 Deployment 的 Pod 模板（<mark style="color:orange;">`.spec.template`</mark>）发生更改时，才会创建新修订版本。&#x20;
...
Deployment 控制器会自动停止有问题的部署过程，并停止对新的 ReplicaSet 扩容。 这行为取决于所指定的 rollingUpdate 参数（具体为 <mark style="color:orange;">`maxUnavailable`</mark>）。 默认情况下，Kubernetes 将此值设置为 25%。
{% endhint %}
