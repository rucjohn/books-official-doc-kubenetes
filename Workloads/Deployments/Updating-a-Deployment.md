...
仅当 Deployment Pod 模板（即 <mark style="color:orange;">`.spec.template`</mark>）发生改变时，才会触发 Deployment rollout 动作；例如，模板的标签或容器镜像被更新。其他更新（如，对 Deployment 执行扩缩容操作）不会触发 rollout 动作。
{% endhint %}
...
说明： 在 apps/v1\` API 版本中，Deployment 标签选择器在创建后是不允许改变的。
